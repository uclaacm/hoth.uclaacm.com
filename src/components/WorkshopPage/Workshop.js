...
function Workshop() {
    ...
    const workshopCards = workshops.map(item =>
        <Grid style={{ paddingBottom: theme.spacing(isSmall ? 4 : 8) }}>
            <Typography style={{ fontWeight: theme.typography.fontWeightMedium, paddingBottom: theme.spacing(isSmall ? 4 : 8) }} variant='h3' component='h2'>{item.type}</Typography>
                ...
                {item.elements.map(element => {
                    return (
                        ...
                        <Grid item xs={12} sm={8} md={6}
                            ...
                            <Typography variant='h4' component='h3' style={{ paddingTop: '0.5em' }}>{element.title}</Typography>
